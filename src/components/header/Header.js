...
//     }
//     render() {
//         const { username, isBuyer, isSeller } = this.context;
...
//                                                 <NavLink to={"/myshop"} className="md:w-full">
...
export default function Header(props) {
    ...
    //context
    const { isBuyer, isSeller, username } = useContext(AuthContext);
    ...
    const handleMouseOver = (event) => {
        ...
        setHoverState({ [event.currentTarget.id]: true });
    }

    const handleMouseLeave = (event) => {
        setHoverState({ [event.currentTarget.id]: false })
    }
        ...
        <div className="w-full flex flex-col z-10 md:sticky md:top-0 justify-center items-center md:shadow-md bg-green-600 md:bg-white">
            <div className="wrapper w-11/12 md:w-9/12 py-3 flex justify-between items-center gap-1">
                ...
                <div className="section md:w-auto md:h-auto flex md:gap-5 justify-end items-center">
                    <div className="md:h-14 flex justify-start items-center gap-2 md:gap-0 text-white md:text-gray-500">
                        ...
                        {isBuyer &&
                            <Fragment>
                                ...
                                {/* cart icon */}
                                <div id="dropdown-cart" className="md:p-2" onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave}>
                                    <button className="flex md:p-1 rounded-sm md:hover:bg-gray-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 block md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                    ...
                                    {hoverState["dropdown-cart"] &&
                                        <Dropdown>
                                            {
                                                data.length > 0 ?
                                                    <div>
                                                        <div className='flex md:justify-between md:pb-2 md:border-b border-gray-200 md:text-sm'>
                                                            <NavLink to={"/cart"} className="md:w-auto">
                                                                <button className=' font-bold'>{`Keranjang(${data.length})`}</button>
                                                            </NavLink>
                                                            <NavLink to={"/cart"} className="md:w-auto">
                                                                <button className=' md:font-bold md:text-xs md:text-green-500'>Lihat Sekarang</button>
                                                            </NavLink>
                                                        </div>
                                                        ...
                                                        {/* loop from data cart */}
                                                        {
                                                            data.map((data, index) => {
                                                                return (
                                                                    <div key={index} className={`md:flex md:py-4 ${data.id === dataLength ? "border-0" : "md:border-b"} md:w-96 border-gray-200`}>
                                                                        <div className='md:w-auto'>
                                                                            <div className='md:w-10 md:h-10'>
                                                                                <img src={data.product.images[0].url} />
                                                                            </div>
                                                                        </div>
                                                                        <div className='md:w-full md:px-2 md:text-sm'>
                                                                            <button className='md:font-bold text-left'>{data.product.name}</button>
                                                                            <p className='md:text-xs'>{`${data.quantity} barang`}</p>
                                                                        </div>
                                                                        <div className='md:w-40 flex justify-end items-center'>
                                                                            <p className='md:text-sm md:font-bold text-right text-red-500'>{`Rp ${RupiahFormat(data.amount)}`}</p>
                                                                        </div>
                                                                    </div>
                                                                );
                                                            })
                                ...
                                <div id="dropdown-notification" className="md:p-2" onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave}>
                                    <button className="flex md:p-1 rounded-sm md:hover:bg-gray-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 block md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                        </svg>
                                    ...
                                    {hoverState["dropdown-notification"] &&
                                        <Dropdown>
                                            <div className="md:w-72">
                                                <p>Wah harus beli</p>
                                                <p>Daripada dianggurin, isi saja dengan barang-barang menarik. Lihat-lihat dulu, siapa tahu ada yang kamu suka!</p>
                                ...
                                <div id="dropdown-chat" className="md:p-2" onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave}>
                                    <button className={`flex md:p-1 rounded-sm md:hover:bg-gray-100 ${hoverState["dropdown-chat"] ? 'md:bg-gray-100' : 'md:bg-white'}`}>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 block md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                                        </svg>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 hidden md:block" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        {/* hamburger button */}
                        <button className="block md:hidden text-white" onClick={onToggleHamburger}>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                            </svg>
                        </button>
                    ...
                    <div className={`inner-section ${hamburger ? 'block' : 'hidden'} w-full h-full absolute md:block md:static top-0 left-0 z-10 bg-white`}>

                        <div className="flex p-4 gap-2 border-b md:hidden">
                            <button onClick={onToggleHamburger}>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                                </svg>
                            </button>
                        ...
                        {isBuyer ?
                            <div className="section md:w-60 flex flex-col md:flex-row-reverse gap-2 md:gap-1 p-4 md:p-0">
                                ...
                                {/* profile section */}
                                <div id="dropdown-profile" className="profile-section w-full md:w-1/2 flex md:justify-center md:items-center gap-2 md:cursor-pointer md:p-1 md:rounded-sm md:hover:bg-gray-100" onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave}>
                                    <div className="w-auto rounded-full">
                                        <div className="w-10 h-10 md:w-7 md:h-7 rounded-full bg-green-400"></div>
                                    </div>
                                    <div className="w-11/12 md:w-20 flex flex-col justify-center">
                                            ...
                                            <div className="md:w-80 md:flex md:flex-col md:gap-2 md:text-sm">
                                                ...
                                                <div className="md:flex md:w-full md:text-gray-500">
                                                    ...
                                                    <div className="md:w-2/5">
                                                        ...
                                                        <p>Toko Favorit</p>
                                                        <p>Pengaturan</p>
                                                        <button className="md:w-full md:py-1 md:text-left md:hover:bg-gray-100" onClick={handleLogout}>Keluar</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </Dropdown>
                                ...
                                {/* store section */}
                                <div id="dropdown-store" className="profile-section w-full md:w-1/2 flex gap-2 md:cursor-pointer md:p-1 md:rounded-sm md:hover:bg-gray-100" onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave}>
                                    <div className="w-auto rounded-full">
                                        <div className="w-10 h-10 md:w-7 md:h-7 rounded-full bg-green-400"></div>
                                    </div>
                                    <div className="w-11/12 md:w-20 flex flex-col justify-center">
                                        <p className="text-sm font-bold md:text-sm md:font-normal md:text-gray-500">{storeName === undefined ? "Store" : storeName}</p>
                                        <p className="md:hidden text-xs">Saldo</p>
                                    </div>
                                    <div className="w-auto flex items-center md:hidden">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <div className="flex justify-between p-4 gap-3">
                                <button className="w-3/6 h-8 md:w-24 rounded-md border border-green-500 text-green-500" onClick={props.onLoginBox}>Masuk</button>
                                <NavLink to="/register">
                                    <button className="w-3/6 h-8 md:w-24 rounded-md text-white bg-green-500">Daftar</button>
                                </NavLink>
                            </div>
